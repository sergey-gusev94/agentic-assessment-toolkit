...
Intake is an agent-assisted manual procedure with a deterministic
checker (docs/design.md, decision 14): the repository ships this brief
and `aat check-course`; the maintainer runs the agent. It is not a
Harbor job — the materials are trusted professor content, the work is
one-time authoring with a human review gate, and interactivity helps —
and per decision 13 the repository itself never launches agents.
...
1. **Dump.** Create `raw/<course_id>/` in the data root and copy in
   ...
   `PU_CHE456_F2025`); `raw/` is read-only from here on.
2. **Run the agent.** From the data root, start an agent session (e.g.
   Codex CLI) with the agent brief below plus a kickoff line naming the
   course:

   ```sh
   codex exec "$(cat <toolkit-repo>/docs/course-intake.md)

   Process course <course_id>: read raw/<course_id>/ and produce
   courses/<course_id>/ per the agent brief above."
   ```

3. **Check.** `aat check-course --course <course_id>` prints three
   tiers — contract violations (exit is nonzero until they are fixed),
   completeness gaps, and the intake notes — plus a coverage summary
   (assessments with materials, percent of the final grade covered).
4. **Review and fill.** Fix violations, fill what the materials could
   ...
   of every grade produced under it. Re-run the checker until clean.
5. **Use.** Everything stays an editable draft until a job first
   records its hash; from that point the artifact is frozen
   (docs/data-conventions.md). Materials arriving later are added by
   re-running intake; existing artifacts are never modified by it.
...
independent of permission); `due` (TOML date); and `excluded` — a
short reason, only for assessments that have no assignment directory
